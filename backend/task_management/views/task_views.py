...
from task_management.services.task_service import create_task_service, delete_task_service, get_leadership_board_service, get_points_service, get_task_list_service, gpt_suggestion_service, summarize_audio_service, update_task_service
...
def summarize_audio(request):
    data,status_code = summarize_audio_service(request.data)
    return Response(data,status=status_code)

@api_view(["Delete"])
@permission_classes([IsAuthenticated])
def delete_task_view(request,task_id):
    data,status_code = delete_task_service(task_id)
    return Response(data,status=status_code)